...
def rolling_time_feature(df, time=8):
    data = df.copy()
    data['rolling_mean_8h_实时电价'] = data['实时电价'].rolling(window=time * 4,closed='left').mean()  # 直接赋值，不使用.loc
    data['rolling_std_8h_实时电价'] = data['实时电价'].rolling(window=time * 4,closed='left').std()
    data['rolling_min_8h_实时电价'] = data['实时电价'].rolling(window=time * 4,closed='left').min()
    data['rolling_max_8h_实时电价'] = data['实时电价'].rolling(window=time * 4,closed='left').max()

    data['rolling_mean_8h_新能源'] = data['新能源'].rolling(window=time * 4,closed='left').mean()  # 直接赋值，不使用.loc
    data['rolling_std_8h_新能源'] = data['新能源'].rolling(window=time * 4,closed='left').std()
    data['rolling_min_8h_新能源'] = data['新能源'].rolling(window=time * 4,closed='left').min()
    data['rolling_max_8h_新能源'] = data['新能源'].rolling(window=time * 4,closed='left').max()

    data['rolling_mean_8h_系统负荷'] = data['系统负荷'].rolling(window=time * 4,closed='left').mean()  # 直接赋值，不使用.loc
    data['rolling_std_8h_系统负荷'] = data['系统负荷'].rolling(window=time * 4,closed='left').std()
    data['rolling_min_8h_系统负荷'] = data['系统负荷'].rolling(window=time * 4,closed='left').min()
    data['rolling_max_8h_系统负荷'] = data['系统负荷'].rolling(window=time * 4,closed='left').max()

    data['rolling_mean_8h_联络线'] = data['联络线'].rolling(window=time * 4,closed='left').mean()  # 直接赋值，不使用.loc
    data['rolling_std_8h_联络线'] = data['联络线'].rolling(window=time * 4,closed='left').std()
    data['rolling_min_8h_联络线'] = data['联络线'].rolling(window=time * 4,closed='left').min()
    data['rolling_max_8h_联络线'] = data['联络线'].rolling(window=time * 4,closed='left').max()

    data['rolling_mean_8h_竞价空间'] = data['竞价空间'].rolling(window=time * 4,closed='left').mean()  # 直接赋值，不使用.loc
    data['rolling_std_8h_竞价空间'] = data['竞价空间'].rolling(window=time * 4,closed='left').std()
    data['rolling_min_8h_竞价空间'] = data['竞价空间'].rolling(window=time * 4,closed='left').min()
    data['rolling_max_8h_竞价空间'] = data['竞价空间'].rolling(window=time * 4,closed='left').max()
    return data
